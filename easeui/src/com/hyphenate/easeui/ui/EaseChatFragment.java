...
//            R.drawable.ease_chat_location_selector };
    ...
    protected int[] itemdrawables = {R.drawable.camera, R.drawable.image,
            R.drawable.marker};
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            ...
            if (EaseUserUtils.getUserInfo(toChatUsername) != null) {
                if (EaseUserUtils.getUserInfo(toChatUsername).getNick().length() != 32)
    ...
    protected void setListItemClickListener() {
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            public void onResendClick(final EMMessage message) {
                new EaseAlertDialog(mActivity, R.string.resend, R.string.confirm_resend, null, new AlertDialogUser() {
                    @Override
                ...
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (listView.getFirstVisiblePosition() == 0 && !isloading && haveMoreData) {
                            ...
                            Toast.makeText(mActivity, getResources().getString(R.string.no_more_messages),
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                if (locationAddress != null && !locationAddress.equals("")) {
                    ...
                    Toast.makeText(mActivity, R.string.unable_to_get_loaction, Toast.LENGTH_SHORT).show();
                }
        ...
        public void onMessageReceived(final List<EMMessage> messages) {
            ...
            for (final EMMessage message : messages) {
                ...
                if (username.equals(toChatUsername)) {
                    ...
                    if (chatType == EaseConstant.CHATTYPE_SINGLE) {
                        String messageFlag = message.getStringAttribute("fire", FIRE_ON);
                        Log.i("tag", "messageFlag:" + messageFlag + ";messageId:" + message.getMsgId());
                        if (messageFlag.equals(FIRE_ON)) {//删除
                            try {
                                Thread.sleep(2000);
...
//        }
    ...
    public void onStop() {
        ...
        EaseUI.getInstance().popActivity(mActivity);
    }
    ...
    protected void onChatRoomViewCreation() {
        final ProgressDialog pd = ProgressDialog.show(mActivity, "", "Joining......");
        EMClient.getInstance().chatroomManager().joinChatRoom(toChatUsername, new EMValueCallBack<EMChatRoom>() {
            ...
            public void onSuccess(final EMChatRoom value) {
                mActivity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (mActivity.isFinishing() || !toChatUsername.equals(value.getId()))
                            return;
                        pd.dismiss();
        ...
        public void onClick(int itemId, View view) {
            ...
            switch (itemId) {
                ...
                case ITEM_LOCATION: // 位置
                    startActivityForResult(new Intent(mActivity, EaseBaiduMapActivity.class), REQUEST_CODE_MAP);
                    break;
    ...
    protected void sendPicByUri(Uri selectedImage) {
        ...
        if (cursor != null) {
            ...
            if (picturePath == null || picturePath.equals("null")) {
                Toast toast = Toast.makeText(mActivity, R.string.cant_find_pictures, Toast.LENGTH_SHORT);
    ...
    protected void sendFileByUri(Uri uri) {
        ...
        if (file == null || !file.exists()) {
            Toast.makeText(mActivity, R.string.File_does_not_exist, Toast.LENGTH_SHORT).show();
            return;
        }
        //大于10M不让发送
        if (file.length() > 10 * 1024 * 1024) {
            Toast.makeText(mActivity, R.string.The_file_is_not_greater_than_10_m, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    protected void selectPicFromCamera() {
        if (!EaseCommonUtils.isExitsSdcard()) {
            Toast.makeText(mActivity, R.string.sd_card_does_not_exist, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    protected void toGroupDetails() {
        if (chatType == EaseConstant.CHATTYPE_GROUP) {
            EMGroup group = EMClient.getInstance().groupManager().getGroup(toChatUsername);
            if (group == null) {
                Toast.makeText(mActivity, R.string.gorup_not_found, Toast.LENGTH_SHORT).show();
                return;
            }
            if (chatFragmentListener != null) {
...
//                        Toast.makeText(mActivity, R.string.you_are_group, Toast.LENGTH_SHORT).show();
                        mActivity.finish();
                    }
                }
            });
...
//        }
        ...
        public void onGroupDestroyed(final String groupId, String groupName) {
            ...
            mActivity.runOnUiThread(new Runnable() {
                public void run() {
                    if (toChatUsername.equals(groupId)) {
//                        Toast.makeText(mActivity, R.string.the_current_group, Toast.LENGTH_SHORT).show();
                        mActivity.finish();
                    }
                }
            });
